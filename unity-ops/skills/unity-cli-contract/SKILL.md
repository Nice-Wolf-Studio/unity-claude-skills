...
| Offering the expensive alternative without pricing it against the deadline the user stated | Warn + price it before they spend it: roughly how long that command takes **on this project**, and the cheapest narrowing that shrinks it — `unity vcs affected --since <ref>` to scope the compare to one revision range, plus whatever the option list you just probed carries. Under a stated clock, "run this instead" is half an answer until the user can tell whether it fits | `unity-invocation` (reasoning, so measured rather than matched) |
